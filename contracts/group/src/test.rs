...
/// Assert the group is live with every member's collateral at stake.
///
/// `onboard` already locked collateral and contributed for each member, so
/// this moves no money. Note the pot is credited at *settlement*, not on
/// contribute, so asserting `get_pot` here would read 0 — what a caller
/// actually depends on is an active group with funded members, which is what
/// this checks. Fails loudly if a future change to `onboard` breaks that.
fn fund_period1(s: &Setup) {
    assert_eq!(
        s.client.get_state().status,
        GroupStatus::Active,
        "group should be active once every member has onboarded"
    );
    for m in s.client.get_members().iter() {
        assert!(
            m.collateral_usdc > 0 || m.collateral_xlm > 0,
            "every member should have collateral locked"
        );
    }
}
...
#[test]
fn emergency_dissolution_by_supermajority() {
    let s = setup();
    onboard(&s, 0, 0);
    onboard(&s, 1, 0);
    onboard(&s, 2, 0);
    ...
    // Member 0 proposes dissolution
    s.client.propose_dissolution(&s.members[0]);
    let prop = s.client.get_dissolution_proposal().unwrap();
    assert_eq!(prop.proposer, s.members[0]);
    assert_eq!(prop.votes_count, 1);
    assert!(!prop.passed);

    // Member 1 votes yes -> 2/3 supermajority reached!
    s.client.vote_on_dissolution(&s.members[1]);
    let prop_after = s.client.get_dissolution_proposal().unwrap();
    assert!(prop_after.passed);
    assert_eq!(s.client.get_state().status, GroupStatus::EmergencyDissolved);

    // All members can withdraw their locked collateral immediately
    s.client.withdraw_collateral(&s.members[0]);
    s.client.withdraw_collateral(&s.members[1]);
    s.client.withdraw_collateral(&s.members[2]);
}

#[test]
fn dissolution_passes_without_unanimity() {
    // Regression: the threshold was `(total * 2) / 3 + 1`, which for 3 members
    // required 3 votes — unanimity — so a single hold-out could veto a
    // dissolution two thirds of the group had already agreed to. A 2/3
    // supermajority must clear with member 2 never voting.
    let s = setup();
    onboard(&s, 0, 0);
    onboard(&s, 1, 0);
    onboard(&s, 2, 0);
    ...
    let prop = s.client.get_dissolution_proposal().unwrap();
    assert!(prop.passed, "2 of 3 is a 2/3 supermajority and must pass");
    assert_eq!(prop.votes_count, 2, "the hold-out must not be counted");
    assert_eq!(
        s.client.get_state().status,
        GroupStatus::EmergencyDissolved
    );
}

#[test]
#[should_panic(expected = "Error(Contract, #")]
fn dissolution_rejects_double_vote() {
    let s = setup();
    onboard(&s, 0, 0);
    onboard(&s, 1, 0);
    onboard(&s, 2, 0);
    ...
    s.client.propose_dissolution(&s.members[0]);
    // The proposer is already recorded as a voter; voting again must fail
    // rather than let one member manufacture a supermajority alone.
    s.client.vote_on_dissolution(&s.members[0]);
}

#[test]
fn exit_forming_safely() {
    let s = setup();
    // Founder locks collateral
    s.client.lock_collateral(&s.members[0], &0);
    assert_eq!(s.client.get_members().len(), 1);

    // Founder exits before group starts
    s.client.exit_forming(&s.members[0]);
    assert_eq!(s.client.get_members().len(), 0);
}
