...
async function main() {
  console.log('=== ONBOARDING USERS #32 TO #51 ON MAINNET ===');
  console.log('Deployer:', deployerKp.publicKey());

  const existingData = JSON.parse(
    readFileSync('C:/Users/vivek/OneDrive/Documents/Projects/Plexa(v1)/scripts/mainnet-users.json', 'utf8')
  );
  const userRecords = existingData.users;

  let acct = await server.getAccount(deployerKp.publicKey());

  for (let i = 32; i <= 51; i++) {
    console.log(`\n--- Processing User #${i} / 51 ---`);
  ...
  writeFileSync(
    ...
    JSON.stringify({ groupAddress, groupCreateTxHash: existingData.groupCreateTxHash, users: userRecords }, null, 2),
    'utf8'
  );

  console.log('\n=== ALL 51 USERS SUCCESSFULLY ONBOARDED ON MAINNET! ===');
}
