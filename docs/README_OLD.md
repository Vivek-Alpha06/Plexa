...
## Feedback sheet: [Google Sheet](https://docs.google.com/spreadsheets/d/1Hc3Hp1LWov_zRv7xerMRvo18IKWWBSK_Kn3P41_JaZU/edit?usp=sharing)
...
> **How this traffic was produced:** these wallets interacted with the contracts
> through create → join → lock collateral → contribute — real transactions
> exercising the real contract paths.
> A handful of the earlier wallets
> (the ones with 7-8 actions each) ran a full ROSCA cycle, including
> auction bidding, payout claims, and collateral withdrawal — see
> [`e2e/e2e.mjs`](e2e/e2e.mjs) for that flow.
>
> Re-read the ledger anytime with:
> ```bash
